...
```{r}
dpr1415 <- dpr %>% filter(Year == "2014-2015")
...
g_dpr1415 <- left_join(g_dpr1415, state_pop, by = c("State" = "State"))
```
...
```{r}
unique(g_dpr1415$State[is.na(g_dpr1415$Population)])
```
...
disp_map <- left_join(g_dpr1415, all_states, by = c("State" = "region"))
```
...
```{r}
ggplot(data = g_dpr1415, aes(x = total)) + 
