...
```{r}
# TODO: Document definitions of variable names or link to variable description
```

```{r}
comb <- comb %>% 
  rename(# Written signed complaints
         wsc_total = `Written, Signed Complaints WSC Total`, 
         wsc_reports = `WSC with Reports Issued Total`, 
         wsc_reports_with_findings = `WSC Reports with Findings`, 
         wsc_reports_ontime  = `WSC Reports within Timeline`, 
         wsc_reports_ext_timelines = `WSC Reports within Extended Timelines`, 
         wsc_pending = `WSC Pending Total`, 
         wsc_pending_due_proc = `WSC Pending a Due Process Hearing`, 
         wsc_withdrawn = `WSC Withdrawn or Dismissed`, 
         # Mediations
         med_total = `Mediation Requests Total`, 
         med_held = `Mediations Held Total`, 
         med_from_due_proc = `Mediations Held Related to Due Process Complaints`, 
         med_agreed_from_due_proc = `Mediation Agreements Related to Due Process Complaints`, 
         med_not_from_due_proc = `Mediations Held Not Related to Due Process Complaints`,
         med_agreed_not_from_due_proc = `Mediation Agreements Not Related to Due Process Complaints`, 
         med_pending = `Mediations Pending`, 
         med_withdrawn = `Mediations Withdrawn or Not Held`, 
         # Due process
         dpc_total = `Due Process Complaints DPC Total`, 
         dpc_resolutions = `DPC Resolution Meetings Total`, 
         dpc_resolutions_settlements = `DPC Resolution Meetings - Written Settlement Agreements`, 
         dpc_adjudicated = `DPC Hearings fully adjudicated Total`, 
         dpc_decisions_ontime = `DPC Written Decisions within Timeline`, 
         dpc_decisions_ext_timeline = `DPC Written Decisions within Extended Timelines`, 
         dpc_pending = `DPC Pending`, 
         dpc_withdrawn = `DPC Withdrawn or Dismissed`, 
         # Expedited due process complaints
         edpc_total = `Expedited Due Process Complaints EDPC Total`, 
         edpc_resolutions = `EDPC Resulted in a Resolution Meeting Total`, 
         edpc_resolutions_settlements = `EDPC Resolution Meetings - Written Settlement Agreements`, 
         edpc_adjudicated = `EDPC Expedited Hearings fully adjudicated Total`, 
         edpc_placement_changes = `EDPC Expedited Hearings - Change of Placement Ordered`, 
         edpc_pending = `EDPC Pending`, 
         edpc_withdrawn = `EDPC Withdrawn or Dismissed`)
```
